...
/* ── Data-driven roster (live from Discord) ── */
var ROSTER = [];
(function() {
...
function refreshRoster() {
  ...
  .then(function(res) {
    if (!res.ok) { alert('Refresh failed: ' + (res.body.error || 'Unknown error')); return; }
    return fetch('/api/roster').then(function(r){return r.json();}).then(function(list) {
      ROSTER = list;
      renderRoster(list);
    }).catch(function() {
      alert('Roster refreshed on server but failed to reload. Please reload the page.');
    });
  }).catch(function() {
    alert('Failed to reach server while refreshing roster.');
  });
}
